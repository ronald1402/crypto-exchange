...
const DepositsList = dynamic(() => import('@openware/opendax-web-sdk').then((mod: any) => mod.DepositsList), {
    ssr: false,
})

const DepositsHistoryList: React.FC = () => {
    const intl = useIntl();
        ...
        return (
            <div className="w-full min-w-max p-5 border-b bg-navbar-background-color sticky top-0 z-10">
                <Tab
                    ...
                    selectedContent={translate('page.body.history.tab.deposit')}
    ...
    return (
        <>
            <Head>
                <title>{appTitle(translate('page.tab.header.deposits'))}</title>
            </Head>
            <Layout>
...
export default withAuth(DepositsHistoryList);
